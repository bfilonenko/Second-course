...
BubbleSort::BubbleSort(vector<int> &values)
{
	AlgorithmParameter algorithm;
...
void BubbleSort::goToNext()
{
	AlgorithmParameter algorithm(*getStruct());

	if (algorithm.indexes[0] > 0)
	{
		...
		if (algorithm.indexForHighlight.size() == 0)
		{
			...
			else
			{
				...
				if (algorithm.mainIndexes[0] < algorithm.indexes[0])
				{
				...
				else
				{
					...
					algorithm.mainIndexes[0] = 1;
					algorithm.mainIndexes[1] = 0;
				}
			}
		}
		else
